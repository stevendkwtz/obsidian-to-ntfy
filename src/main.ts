...
async function collectTasksFromFiles(app: App): Promise<Task[]> {
    ...
    for (const file of app.vault.getFiles()) {
        ...
        while ((match = taskRegex.exec(content)) !== null) {
            ...
            const dueDateMatch = /📅 ([0-9]{4}-[0-9]{2}-[0-9]{2})/.exec(remainder);
            const priorityMatch = /(⏫|🔼)/.exec(remainder);
            const recurrenceMatch = /🔁 every (day|week|month|year)/.exec(remainder);
            ...
            const taskTags = match[2].match(/#\w+/g) || [];
            const descriptionWithoutTags = match[2].replace(/#\w+/g, '').trim();
...
async function notifyTask(
    ...
    subscription: string,
    app: App,
    _pluginSettings: ObsidianToNtfySettings,
): Promise<void> {
    let clickUrl: string | null = null;
    if (task.fileName && task.lineNumber) {
        const currentVault = app.vault.getName();
        clickUrl = `obsidian://open?vault=${encodeURIComponent(currentVault)}&file=${encodeURIComponent(
            task.fileName,
...
async function shouldNotify(
    ...
    pluginSettings: ObsidianToNtfySettings,
): Promise<boolean> {
...
async function checkTasksAndNotify(
    app: App,
    ntfySubscriptions: { [key: string]: string },
    pluginSettings: ObsidianToNtfySettings,
): Promise<void> {
    ...
    async onload() {
        ...
        this.registerInterval(
            window.setInterval(() => {
                ...
                if (Object.keys(ntfySubscriptions).length > 0) {
                    // Check if tagSubscriptions are set
                    checkTasksAndNotify(this.app, ntfySubscriptions, this.settings);
                }
            }, 40000),
        ); // Adjust this interval as you see fit
    }
...
class ObsidianToNtfySettingTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(containerEl)
            ...
            .addToggle((toggle) =>
                toggle.setValue(this.plugin.settings.enableInAppNotifications || false).onChange(async (value) => {
        ...
        new Setting(containerEl)
            ...
            .addText((text) =>
                text
                    .setPlaceholder('30')
                    .setValue(
                        this.plugin.settings.notificationTimeout
                            ? (this.plugin.settings.notificationTimeout / 60000).toString()
                            : '30',
                    )
                    .onChange(async (value) => {
    ...
    setupLayout(containerEl: HTMLElement) {
        ...
        containerEl.createEl('p', {
            text: 'This plugin allows you to map Obsidian tags to ntfy subscriptions. When a task with a specific tag reaches its due date, a notification will be sent out to all the listeners of the corresponding ntfy subscription.',
        });

        containerEl.createEl('p', {
            text: 'For example, if you have a task tagged with "#work", and you map "#work" to a specific ntfy subscription, then all listeners of that subscription will be notified when the task is due.',
        });

        // Add hyperlink to ntfy
        const ntfyLink = containerEl.createEl('a', {
            href: 'https://ntfy.sh/app',
            text: 'Learn more about ntfy subscriptions.',
        });
        ...
        const obsidianTasksLink = containerEl.createEl('a', {
            href: 'https://publish.obsidian.md/tasks/Introduction',
            text: 'Learn more about Obsidian Tasks.',
        });
    ...
    createSettingInput(containerEl: HTMLElement, initialTag: string, initialUrl: string, isNew = false) {
        let currentTag = initialTag;
        const currentSubscription = initialUrl;
